...
const handler: Handler = async (event, context) => {
  ...
  if (segements.length) return await getOne({ segements, verbose, collection: "characters" })

  /** Curate the queryStringParameters that are avaliable */
  const avaliableFilters = ['name', 'species', 'location', 'origin', 'status', 'gender', 'episode', 'id']
  ...
  return await getMany({ size, after, before, params, verbose, collection: "characters" })
}
