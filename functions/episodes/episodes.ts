...
const handler: Handler = async (event, context) => {
  ...
  if (segements.length) {
    ...
    const record = await getOne({ segements, verbose, collection: "episodes" })
    return record.addResponseTime(t1 - t0)
  }

  // Curate the queryStringParameters that are avaliable
  const avaliableFilters = ['name', 'date', 'id', 'season']
  ...
  const response = await getMany({ 
    ...
    verbose, 
    collection: "episodes", 
    request: getOriginalRequest("episodes", event.queryStringParameters)
  })
